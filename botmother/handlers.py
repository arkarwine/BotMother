...
def build_application(token: str, db: Database, service: BotService):
    ...
    async def delete_message_safely(message: Any) -> None:
        try:
            ...
            logger.debug("Could not delete temporary progress message", exc_info=True)
    ...
    async def finalize_streamed_html(
        ...
        return await reply_html(update.effective_message, text)
    ...
    def new_stream_draft_id(context: ContextTypes.DEFAULT_TYPE) -> int:
        draft_id = uuid.uuid4().int % 2_147_483_647
        if draft_id == 0:
            draft_id = 1
        context.user_data["_botmother_last_draft_id"] = draft_id
        return draft_id
    ...
    async def send_plain_draft(
        ...
        *,
        ...
        try:
            ...
            logger.debug(
                "Telegram draft stream tick accepted: chat_id=%s draft_id=%s chars=%s",
                _chat_id(update),
                draft_id,
                len(text),
            )
            ...
            logger.debug("Telegram draft stream update failed", exc_info=True)
            return False

    def code_tail_text(text: str, max_lines: int = 14, max_chars: int = 2600) -> str:
        lines = text.strip().splitlines()
        tail = "\n".join(lines[-max_lines:]) if lines else text.strip()
        if len(tail) > max_chars:
            tail = "...\n" + tail[-max_chars:]
        return tail.strip() or "..."

    def code_tail_progress_text(
        update: Update,
        title_key: str,
        detail_key: str,
        elapsed_seconds: int,
        ...
        return (
            ...
            + "\n\n<pre>"
            + escape(code_tail_text(text))
            + "</pre>"
        )

    def code_tail_draft_text(
        update: Update,
        title_key: str,
        detail_key: str,
        elapsed_seconds: int,
    ...
    async def send_coding_brief_file(
        update: Update,
        brief: str,
        title_key: str = "ai.coding_brief_title",
        filename: str = "botmother-coding-prompt.txt",
    ) -> None:
        text = brief.strip()
        if not text:
            return
        ...
        await update.effective_message.reply_document(
            document=document,
            filename=filename,
            caption=tr(update, title_key),
            parse_mode=ParseMode.HTML,
        )

    def drain_stream_queue(
        stream_queue: asyncio.Queue[str], streamed_parts: list[str]
    ) -> int:
        received = 0
        while True:
            try:
        ...
        while not task.done() or (stream_queue is not None and not stream_queue.empty()):
            if stream_queue is None:
                ...
                try:
                    ...
                    pass
                received_chars += drain_stream_queue(stream_queue, streamed_parts)
        ...
        if stream_visible_text and streamed_parts:
            ...
            if rendered != last_rendered:
                if stream_mode == "code_tail":
                    ...
                    await send_plain_draft(update, context, rendered, draft_id=draft_id)
        return await task
    ...
    async def newbot_token(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if existing is not None:
            ...
            return NEW_TOKEN
        await send_coding_brief_file(
            update,
            str(decision.code or ""),
            filename="botmother-newbot-coding-prompt.txt",
        )
        progress_message = await reply_html(
            update.effective_message,
            progress_text(
        ...
        try:
            code_stream_queue, code_on_delta = make_stream_queue()
            result = await run_with_progress(
                ...
                service.create_bot_from_decision(
                    ...
                    decision,
                    user_context=str(context.user_data.get("newbot_user_context", "")),
                    on_delta=code_on_delta,
                ),
                "ai.progress_codegen_title",
                "ai.progress_codegen_detail",
                max_tokens=service.settings.openrouter_coding_max_tokens,
                stream_queue=code_stream_queue,
                stream_visible_text=True,
                stream_mode="code_tail",
            )
        except Exception:
    ...
    async def answer_bot_question(
        ...
        while not result_task.done() or not stream_queue.empty():
            try:
                ...
                streamed_parts.append(delta)
                drain_stream_queue(stream_queue, streamed_parts)
            except asyncio.TimeoutError:
                delta = ""
            ...
            if preview == last_preview or (
                last_preview and now - last_update < draft_interval
            ):
                continue
            ...
            await send_plain_draft(update, context, streamed_text, draft_id=draft_id)

        final_preview = "".join(streamed_parts).strip()
        if final_preview and final_preview != last_preview:
            streamed_text = stream_preview_with_usage(
                update,
                final_preview,
                max_tokens=service.settings.openrouter_interaction_max_tokens,
    ...
    async def continue_edit_planning(
        ...
        await send_coding_brief_file(
            update,
            str(decision.code or ""),
            title_key="ai.edit_brief_title",
            filename="botmother-edit-coding-prompt.txt",
        )
        await edit_message_html(
            progress_message,
            progress_text(
